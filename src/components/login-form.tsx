...
export function LoginForm({ setIsGuest }: guestProps) {
    ...
    useEffect(() => {
        ...
        newErrors.cardNumber = `${t('card_number_length')}`
        } else if (cardNumber && !/^\d+$/.test(cardNumber)) {
        newErrors.cardNumber = `${t('card_number_invalid')}`
        }
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                handleToast(`${t('login_success')}`, "success");
                ...
                handleToast(`${t('login_failed')}`, "error");
                setIsLoading(false);
            }
        } catch (error: any) {
    ...
    const handleEnglishToJapaneseTranslate = async (key: string) => {
    ...
    const handleGuestLogin = () => {
        handleToast(`${t('continuing_as_guest')}`, "info");
    ...
    <I18nextProvider i18n={i18n}>
        <div className="space-y-6 z-10">
            ...
            <div className="text-center space-y-4">
                ...
                <div>
                    <h1 className="text-3xl font-bold text-white mb-2">{t('ai_librarian')}</h1>
                    <p className="text-blue-200 text-lg">{t('digital_knowledge_assistant')}</p>
                    <p className="text-blue-300 text-sm mt-1">{t('personalized_library_experience')}</p>
                </div>
            </div>
            {/* Login Card */}
                ...
                <div className="space-y-6 px-6">
                    ...
                    <form onSubmit={handleLogin} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-blue-300">{t('card_hint')}</p>
                        ...
                        >
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <span>{t('accessing_library')}</span>
                            </div>
                            ) : (
                            <div className="flex items-center space-x-2">
                                <BookOpen className="w-4 h-4" />
                                <span>{t('access_library')}</span>
                            </div>
                            )}
                        </button>
                        {/* Sign Up Link */}
                        <div className="text-center">
                            <span className="text-blue-200 text-sm">{t('no_card')}</span>
                            <button
                            type="button"
                            // onClick={}
                            className="cursor-pointer text-orange-400 hover:text-orange-300 text-sm font-medium transition-colors"
